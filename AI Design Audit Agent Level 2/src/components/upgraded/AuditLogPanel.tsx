...
export function AuditLogPanel({ logs, isLoading }: AuditLogPanelProps) {
  ...
  const getLogStyle = (log: string) => {
    ...
    if (text.includes("error") || text.includes("fail") || text.includes("crash") || text.includes("aborted")) {
      return "text-rose-400";
    }
    if (text.includes("complete") || text.includes("success") || text.includes("passed") || text.includes("finished")) {
      return "text-emerald-400";
    }
    if (text.includes("running") || text.includes("analyzing") || text.includes("extracting") || text.includes("comparing")) {
      return "text-cyan-400";
    }
    if (text.includes("warning") || text.includes("alert") || text.includes("discrepanc") || text.includes("regression")) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-[11px] leading-relaxed custom-scrollbar bg-slate-950/80 min-h-[200px]">
        {logs.length === 0 ? (
          <div className="text-slate-500 italic h-full flex items-center justify-center">
            Upload screenshots to initialize execution console trace...
          </div>
        ) : (
          logs.map((log, index) => (
        ...
        {isLoading && (
          <div className="flex items-center space-x-2 text-cyan-400 font-mono animate-pulse mt-3 pl-4">
            ...
            <span>AI comparative vision parser running...</span>
          </div>
        )}
