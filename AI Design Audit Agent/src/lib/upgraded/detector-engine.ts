...
export class ContrastDetector implements IDetector {
  ...
  confidence(): number {
    return 98; // Contrast calculation is highly deterministic
  }
}
...
export class AlignmentDetector implements IDetector {
  async analyze(elements: RawLayoutElement[]): Promise<DetectionResult[]> {
    ...
    // Group elements by approximate alignment (e.g. left X coordinate within 1.5% tolerance)
    const groups: { [key: number]: RawLayoutElement[] } = {};
    const tolerance = 1.5;

    for (const el of elements) {
      if (el.type === "container") continue; // Skip full background containers
    ...
    // Check groups for misalignment offsets
    for (const groupKeyStr of Object.keys(groups)) {
...
export class SpacingDetector implements IDetector {
  async analyze(elements: RawLayoutElement[]): Promise<DetectionResult[]> {
    ...
    // Sort elements vertically to check grid gap sizes
    ...
    for (let i = 0; i < sorted.length - 2; i++) {
      ...
      // If gaps are close but uneven (e.g. gap difference is between 0.5% and 3% screen height)
  ...
  async analyze(elements: RawLayoutElement[]): Promise<DetectionResult[]> {
    ...
    for (const el of elements) {
      if (el.type === "text" && el.fontSize) {
        // Accessibility alert: text smaller than 12px
        if (el.fontSize < 12) {
          const result: DetectionResult = {
            id: `type-size-${el.id}`,
            category: "Typography",
...
export class HierarchyDetector implements IDetector {
  async analyze(elements: RawLayoutElement[]): Promise<DetectionResult[]> {
    ...
    // Find headers and body text
    const headers = elements.filter(el => el.type === "header");
    const texts = elements.filter(el => el.type === "text");

    for (const header of headers) {
      for (const text of texts) {
        // If body text size is equal or greater than header font size
...
export class AccessibilityDetector implements IDetector {
  async analyze(elements: RawLayoutElement[]): Promise<DetectionResult[]> {
    ...
    for (const el of elements) {
      // Check input fields missing labels/placeholders, or buttons too small for touch targets
      if (el.type === "button" && el.box.height < 5) { // 5% of height is ~40px on standard screen heights
        const result: DetectionResult = {
...
export function deriveConfidence(ocrClarity: number, evidenceCertainty: number, detectorAgreement: number): number {
  // Confidence = 40% OCR clarity + 30% contrast certainty + 30% rule-matching agreement
